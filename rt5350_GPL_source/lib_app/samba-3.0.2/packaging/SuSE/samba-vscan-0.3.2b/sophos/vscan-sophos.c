/*
 * $Id: vscan-sophos.c,v 1.44 2003/03/29 09:24:12 reniar Exp $
 * 
...
/* module version */
static const char module_id[]="vscan-sophos 0.3.2b";
...
#endif
{
	...
	vscan_syslog("INFO: connect to service %s by user %s\n", 
	  ...
	  if( !*opts_str ) {
		DEBUG(3, ("vscan-sophos: no configuration file set - using default value (%s).\n", lp_vfs_options(SNUM(conn))));
	  } else {
		p = opts_str;
...
#endif
{
	  ...
	  struct vfs_ops tmp_ops;
        ...
        openlog("smbd_vscan_sophos", LOG_PID, SYSLOG_FACILITY);
