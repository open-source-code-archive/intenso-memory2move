/*
 * $Id: vscan-trend.c,v 1.44 2003/03/29 09:24:12 reniar Exp $
 * 
 ...
 * - Simon Harrison for his solution without Samba VFS (http://www.smh.uklinux.net/linux/trend.html)
...
/* module version */
static const char module_id[]="vscan-trend 0.3.2b";
...
#endif
{
	...
	vscan_trend_init();
	  ...
	  if( !*opts_str ) {
		DEBUG(3, ("vscan-trend: no configuration file set - using default value (%s).\n", lp_vfs_options(SNUM(conn))));
	  } else {
		p = opts_str;
...
#endif
{
	...
	if ( !scan_on_open ) {
        ...
        if ( (default_vfs_ops.stat(conn, fname, &stat_buf)) != 0 ) {   
		/* an error occured */
		DEBUG(5, ("samba-vscan - open: stat failed. Error is %s\n", strerror(errno))); 
		return default_vfs_ops.open(conn, fname, flags, mode);

	} else if ( S_ISDIR(stat_buf.st_mode) ) {
		/* is it a directory? */
		DEBUG(5, ("samba-vscan - open: refused to scan directory\n"));
		 return default_vfs_ops.open(conn, fname, flags, mode);
	} else if ( stat_buf.st_size == 0 ) {
		/* do not scan empty files */
		DEBUG(5, ("samba-vscan - open: refused to scan empty file %s\n", fname));
		 return default_vfs_ops.open(conn, fname, flags, mode);
		...
		return default_vfs_ops.open(conn, fname, flags, mode);
               ...
               /* open socket */
                ...
                if ( sockfd == -1 && deny_access_on_error ) {
                        ...
                        vscan_syslog("ERROR: can not communicate to Trophie - access to file %s denied", fname);
			errno = EACCES;
                        return -1;
                } else if ( sockfd >= 0 ) {
...
#endif
{
	  ...
	  struct vfs_ops tmp_ops;
        ...
        openlog("smbd_vscan_trend", LOG_PID, SYSLOG_FACILITY);
