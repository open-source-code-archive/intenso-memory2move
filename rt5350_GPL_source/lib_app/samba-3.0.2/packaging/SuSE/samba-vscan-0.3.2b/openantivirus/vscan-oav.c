/* 
 * $Id: vscan-oav.c,v 1.34 2003/03/29 09:24:12 reniar Exp $
 *
 * virusscanning VFS module for samba.  Log infected files via syslog
 * facility and block access using OpenAntiVirus ScannerDaemon.
 *
...
/* module version */
static const char module_id[]="vscan-oav 0.3.2b";
...
#endif
{
	...
	/* scan only file, do nothing */	
		...
		rv = vscan_oav_scanfile(sockfd, filepath, client_ip);
		vscan_oav_end(sockfd);
		if ( rv == 1 ) {
			/* virus was found */
...
#endif
{
	  ...
	  struct vfs_ops tmp_ops;
        ...
        openlog("smbd_vscan_oav", LOG_PID, SYSLOG_FACILITY);
