...
void OperationSelect(void)
{
	...
	printf("   %d: Enter IOVST ui menu. \n", SEL_LOAD_BOOT_UIMENU);
 ...
 ************************************************************************
 */
static vs_boot_from_spi(void)
{
	char cmd[256];

	raspi_read((char *)VS_RAM_KERNEL_ADDR, (char *)SPI_KERN_ADDR, SPI_KERN_SIZE);
...
//	vs_cfg_flush();
	...
	printf("Input i key to enter menu:  ");
	vs_break_flag = 0;
	while (timer1 > 0) {
		--timer1;
		/* delay 100 * 10ms */
...
void config_usb_ehciohci(void)
{
	...
	printf("Initialize usb ehci ohci\n");
